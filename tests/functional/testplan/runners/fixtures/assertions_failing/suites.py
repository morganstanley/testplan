...
class MySuite:
    ...
    def test_table_match(self, env, result):
        ...
        result.table.match(actual=table, expected=table[:-1] + [["xxx", 4]])

        result.table.match(
            actual=table, expected=table[:-1] + [[always_false, 4]]
        )

        result.table.match(
            actual=table, expected=table[:-1] + [[re.compile(r"zzz"), 4]]
        )

        result.table.match(
            actual=table, expected=table[:-1] + [[error_func, 4]]
        )
    ...
    def test_table_diff(self, env, result):
        ...
        result.table.diff(actual=table, expected=table[:-1] + [["xxx", 4]])
    ...
    def test_xml_check(self, env, result):
        ...
        # Failing assertions
        result.xml.check(
            element="<Root><Test>Foo</Test></Root>", xpath="/Root/Bar"
        )
        ...
        result.xml.check(
            element="<Root><Test>Value1</Test><Test>Value2</Test></Root>",
            xpath="/Root/Test",
            tags=[cmp.In(["a", "b", "c"]), re.compile("Foo")],
        )
