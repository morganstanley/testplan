...
class TestReport(object):
    ...
    def test_filter(self):
        ...
        rep_copy_2 = rep.filter(
            lambda val: not isinstance(val, int)
        ).filter(
            lambda val: len(val) > 1,
            lambda val: 'hello' in val,
        )
...
class TestReportGroup(object):
    ...
    def test_build_index_recursive(self):
        ...
        child_1, child_2, child_3, child_4 = [DummyReport() for idx in range(4)]
    ...
    def test_merge_children(self):
        ...
        parent_orig = DummyReportGroup(uid=0, entries=[child_orig_1, child_orig_2])
        ...
        with mock.patch.object(child_orig_1, 'merge'):
            with mock.patch.object(child_orig_2, 'merge'):
                ...
                child_orig_1.merge.assert_called_once_with(child_clone_1, strict=True)
                child_orig_2.merge.assert_called_once_with(child_clone_2, strict=True)
    ...
    def test_filter(self):
        ...
        root = DummyReportGroup(name='root', entries=[group_1, group_2, group_3])
