...
def handler(p1: mp.Process, p2: mp.Process, mask_sigterm):
    def _(signum, frame):
        ...
        if IS_WIN:
            ...
            os.kill(p2.pid, signal.CTRL_BREAK_EVENT)
        else:
...
def dummy_loop(mask_sigterm):
    if mask_sigterm:
        if IS_WIN:
            signal.signal(signal.CTRL_BREAK_EVENT, child_handler)
        else:
...
def main(mask_sigterm, sleep_time):
    ...
    if IS_WIN:
        signal.signal(signal.CTRL_BREAK_EVENT, handler(p1, p2, mask_sigterm))
    else:
