...
def mock_reload_env():
    ...
    with mock.patch("modulefinder.ModuleFinder", new=MockModuleFinder), (
        ...
        reloader._GraphModuleFinder.__bases__ = (
            MockModuleFinder, logger.Loggable)
...
def test_dependency_reload(mock_reload_env):
    ...
    reload_obj.reload(tests=[])
    mock_reload.assert_has_calls([mock.call(MOCK_SYSMODULES["mod_b"]),
    ...
    mock_reload.reset_mock()
    reload_obj.reload(tests=[])
    mock_reload.assert_has_calls([mock.call(MOCK_SYSMODULES["mod_c"]),
                                  mock.call(MOCK_SYSMODULES["mod_a"])])
    ...
    mock_reload.reset_mock()
    reload_obj.reload(tests=[])
    mock_reload.assert_has_calls([mock.call(MOCK_SYSMODULES["mod_d"]),
...
def _check_dep_graph(dep_graph):
    ...
    assert dep_graph.graph_string == """__main__ ->
  mod_a ->
    mod_b ->
      mod_d
    mod_c
