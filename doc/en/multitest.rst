...
in test report and displayed on UI. Argument ``name`` is available in @testcase and
should be a normal string, while in @testsuite it can be a normal string or a callable.
To customize names for parametrized testcases another argument ``name_func`` is useful,
refer to the document of :ref:`name_func <parametrization_custom_name_func>`.
Some examples:

    * @testcase(name="My Testcase")
    * @testcase(name="My Testcase", name_func=lambda func_name, kwargs: func_name.upper())
    * @testsuite(name="My Test Suite")
    * @testsuite(name=lambda cls_name, suite: "{} -- {}".format(cls_name, id(suite)))

.. note::

    It is strongly suggested that no duplicate display names of test suites exist in one
    Multitest, argument ``name`` in @testsuite can be useful here with a callable passed.
    By the way, at most 255 characters are allowed for a display name of test suite, or
    Testplan will print warnings.
...
When you use parametrization, Testplan will try to generate a function name for
...
Each parametrized testcase also have a legible name used for display (usually called
display name), A Testplan user only needs to pay attention to the display name which
is automatically generated by default, or can be customized if needed.

In the example below, 2 new testcases will be generated, with the function names being
``'add_list__number_list_[1, 2, 3]__expected_6'`` and
``'add_list__number_list_[6, 7, 8, 9]__expected_30'``. These are not valid Python
attribute names, so Testplan will fall back to ``'add_list__0'`` and ``'add_list__1'``.
And 2 diaplay names for testcase reports are ``'add_list <number_list=[1, 2, 3], expected=6>'``
and ``'add_list <number_list=[6, 7, 8, 9], expected=30>'``, that is easy to understand.
...
However you can provide custom name generation functions to override this default
functionality via ``@testcase(parameters=..., name_func=custom_name_func)`` syntax.
You just need to implement a function that accepts
...
.. code-block:: python
  ...
  def custom_name_func(func_name, kwargs):
      """
      Will generate names like: "Add List -- (numbers: [1 2 3], result: 6)"
      """
      number_list = kwargs['number_list']

      return '{func_name} -- (numbers: [{joined_list}], result: {expected})'.format(
          func_name=func_name,
          joined_list=' '.join(map(str, number_list)),
...
In the above example, if you set ``name_func`` to None, then the display names generated
are simply ``'Add List 0'`` and ``'Add List 1'``, without any argument showed.

.. note::

  Testplan can make sure that no duplicate function names of testcases exist in one
  test suite by implicitly adding integer suffixes, but no such limit for display names.
  By the way, at most 255 characters are allowed for a display name of testcase, or
  Testplan will print warnings.
