"""
Copied from https://github.com/gjhiggins/tempita
LICENSE: https://github.com/gjhiggins/tempita/blob/master/docs/license.txt
...
class HTMLTemplate(Template):
    ...
    default_namespace.update(
        dict(html=html, attr=attr, url=url, html_quote=html_quote)
    )
...
def lex(s, name=None, trim_whitespace=True, line_offset=0, delimeters=None):
    ...
    for match in token_re.finditer(s):
        ...
        if expr == delimeters[0] and in_expr:
            ...
            raise TemplateError(
                "%s outside expression" % delimeters[1],
                position=pos,
                name=name,
            )
...
def trim_lex(tokens):
    ...
    for i in range(len(tokens)):
        ...
        if not isinstance(next_chunk, basestring_) or not isinstance(
            prev, basestring_
        ):
            continue
...
def parse_expr(tokens, name, context=()):
    ...
    if expr.startswith("py:"):
        ...
        if "for" not in context:
            raise TemplateError(
                "continue outside of for loop", position=pos, name=name
            )
        ...
        raise TemplateError(
            "%s with no expression" % expr, position=pos, name=name
        )
...
def parse_for(tokens, name, context):
    ...
    if not match:
        raise TemplateError(
            'Bad for (no "in") in %r' % first, position=pos, name=name
        )
...
def parse_def(tokens, name, context):
    ...
    while 1:
        if not tokens:
            raise TemplateError(
                "Missing {{enddef}}", position=start, name=name
            )
...
def fill_command(args=None):
    ...
    parser = optparse.OptionParser(
        version=coerce_text(dist), usage=_fill_command_usage
    )
    parser.add_option(
        "-o",
        "--output",
