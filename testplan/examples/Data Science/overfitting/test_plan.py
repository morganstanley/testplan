...
class ModelExamplesSuite(object):
    ...
    def setup(self, env, result):
        ...
        self.X, self.y = np.loadtxt('cos_data.csv', delimiter=',', unpack=True,
                                    skiprows=1)
        ...
        data = [['X', 'y']] + [[self.X[i], self.y[i]]
                               for i in range(len(self.X))]
        result.table.log(data, description='Raw cosine data')

    @testcase(parameters={'degrees': [2, 3, 4, 5, 10, 15]},
