...
describe("ExtendedSearchDropdown", () => {
  ...
  const renderDropdown = () => {
    if (!mountedDropdown) {
      mountedDropdown = shallow(
        <ExtendedSearchDropdown {...props} />
      );
    }
  ...
  it("renders test, testsuite, and testcase sections", () => {
    const dropdown = renderDropdown();
    const labels = dropdown.find(".extended-search-label");
  ...
  it("keeps the selected testcase and opens all options on focus", () => {
    ...
    expect(dropdown.find(".extended-search-testcase-option")).toHaveLength(1);
  ...
  it("selects a testcase from the dropdown", () => {
    ...
    dropdown
      .find(".extended-search-testcase-option")
      .simulate("click");
  ...
  it("does not infer parameters from a historical v4 report", () => {
    ...
    expect(dropdown.find(".extended-search-param-label")).toHaveLength(0);
    expect(dropdown.find(".extended-search-result-item")).toHaveLength(2);
  });

  it("uses structured parameters from a v5 report", () => {
    props.report = parametrizedReport(5, [
      {
        category: "testcase",
        name: "test_order 0",
        uid: "test_order__0",
    ...
    const dropdown = renderDropdown();
    const labels = dropdown.find(".extended-search-param-label");
  ...
  it("navigates using the propagated UID chain", () => {
    ...
    const dropdown = renderDropdown();
    dropdown.find(".extended-search-result-item").simulate("click");
  ...
  it("uses the full UID chain for result keys", () => {
    ...
    const secondMultitest = JSON.parse(
      JSON.stringify(props.report.entries[0])
    );
    secondMultitest.name = "MyTest2";
    secondMultitest.uid = "mt2";
    secondMultitest.entries[0].entries[0].entries[0].uids[1] =
      "mt2";
    props.report.entries.push(secondMultitest);
    props.persistedState.selectedTestcase = "test_order";

    const resultItems = renderDropdown().find(
      ".extended-search-result-item"
    );

    expect(resultItems.map((item) => item.key())).toEqual([
      "report-v5/mt1/ts1/test_order/test_order__0",
      "report-v5/mt2/ts1/test_order/test_order__0",
    ]);
  });
});
...
describe("ExtendedSearchDropdown helpers", () => {
  ...
  describe("parameter value formatting", () => {
    it("uses type-aware tokens", () => {
      ...
      expect(paramValueToken(false)).not.toBe(
        paramValueToken("false")
      );
    });
  ...
  describe("applyParamFilters", () => {
    ...
    it("filters by multiple params", () => {
      const result = applyParamFilters(
        perms, { side: "BUY", qty: "100" }
      );
      expect(result).toHaveLength(1);
    });

    it("treats empty string as 'Any'", () => {
      const result = applyParamFilters(
        perms, { side: "" }
      );
      expect(result).toHaveLength(3);
    ...
    it("distinguishes structured values by type", () => {
      ...
      expect(
        applyParamFilters(typed, { value: 1 })
      ).toEqual([typed[0]]);
      expect(
        applyParamFilters(typed, { enabled: false })
      ).toEqual([typed[0]]);
    });
  });
});
