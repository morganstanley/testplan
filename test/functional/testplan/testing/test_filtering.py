...
from testplan.common.utils.testing import (
    log_propagation_disabled, argv_overridden, check_report_context
)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            ...
            (
                filtering.Pattern('*:*:test_two') and
                filtering.Tags({'color': 'blue'})
            ),
            [
                ('XXX', [
...
def test_programmatic_filtering(filter_obj, report_ctx):
    multitest_x = MultiTest(
        name='XXX', suites=[Alpha(), Beta()])
    multitest_y = MultiTest(
        name='YYY', suites=[Gamma()])
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            ...
            (
                '--tags', 'bar color=blue',  # bar OR color=blue
                '--tags-all', 'baz color=red'  # baz AND color=red
            ),
            [
                ('XXX', [
                    ('Alpha', ['test_two']),
