...
INEQUALITY_PARAMS = [
                        ...
                        ({}, {1, 2}),
                        (123, 321),
                    ] + LESS_THAN_PARAMS + GREATER_THAN_PARAMS
...
def generate_regex_test(assertion_kls, passing_params, failing_params):
    ...
    class RegexTest(BaseRegexTest):
        ...
        def test_evaluate_true(
            ...
            self._test_evaluate(
                regexp=regexp, string=string, flags=flags,
                expected_match_indexes=expected_match_indexes, expected=True)
        ...
        def test_evaluate_false(
            ...
            self._test_evaluate(
                regexp=regexp, string=string, flags=flags,
                expected_match_indexes=expected_match_indexes, expected=False)
...
REGEX_MATCH_LINE_PASS_PARAMS = [
    ...
    (
        'foo',
        multiline('foobar', 'baz', line_sep=os.linesep),
        0,
        [(0, 0, 3)]
    ),
]
...
REGEX_FIND_ITER_PARAM_NAMES = 'regexp,string,flags,' \
                              'condition,expected_match_indexes'
...
class TestRegexFindIter(BaseRegexTest):
    ...
    def test_evaluate_true(
        ...
        self._test_evaluate(
            regexp=regexp, string=string, flags=flags, condition=condition,
            expected_match_indexes=expected_match_indexes, expected=True)
...
def generate_equal_slices_test(assertion_kls, passing_params, failing_params):
    ...
    class EqualSlicesTest(object):
        ...
        def _test_evaluate(
            ...
            slices, expected_data, expected_result
        ):
        ...
        def test_evaluate_true(self, actual, expected, slices, expected_data):
            self._test_evaluate(
                actual=actual, expected=expected, slices=slices,
                expected_data=expected_data, expected_result=True)
        ...
        def test_evaluate_false(self, actual, expected, slices, expected_data):
            self._test_evaluate(
                actual=actual, expected=expected, slices=slices,
                expected_data=expected_data, expected_result=False)
...
EXCEPTION_RAISED_PARAM_NAMES = 'raised_exception,' \
                               'expected_exceptions,pattern,func'
...
def generate_exception_raised_test(
    ...
    class TestException(object):
        ...
        def _test_evaluate(
            ...
            expected_exceptions, pattern, func, expected
        ):
            assertion = assertion_kls(
            ...
            assert list(
                assertion.expected_exceptions) == list(expected_exceptions)
            assert assertion.pattern is pattern
...
class TestColumnContains(object):
    ...
    def _test_evaluate(
        self, table, values, column, limit,
        report_fails_only, expected, expected_data
    ):
    ...
    def test_evaluate_true(
        self, table, expected_data, values,
        column, limit, report_fails_only
    ):
        self._test_evaluate(
            table=table,
...
GET_COMPARISON_COLUMNS_PARAM_NAMES = 'table_1,table_2,' \
                                     'include_columns,exclude_columns,expected'
...
GET_COMPARISON_COLUMNS_ERROR_PARAM_NAMES = 'table_1,table_2,' \
...
TABLEMATCH_COLUMN_NAMES = 'table,expected_table,include_columns,' \
...
class TestXMLCheck(object):
    ...
    def _test_evaluate(
        self, element, xpath,
        tags, namespaces, expected_result
    ):
