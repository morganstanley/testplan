...
@cli.command()
@click.option("--unreleased/--released-only", is_flag=True)
@click.option("--dry-run", is_flag=True)
@click.option("--only-new", is_flag=True)
@click.pass_context
def generate(
    ctx: click.Context, dry_run: bool, only_new: bool, unreleased: bool
):
    ...
    news_file_content = update_news_file(
        news_str, news_file, context.config.insert_marker
    )

    if dry_run:
        print(news_file_content)
        return

    with open(news_file, "wt") as file:
        file.write(news_file_content)
